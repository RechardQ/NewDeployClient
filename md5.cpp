...
bool MD5_CTX::GetFileMd5(char *pMd5,  const char *pFileName)
{
	...
	__int64 length = 0;
	length = _ftelli64(pFile);
	__int64 readlength = 0;
	//unsigned char *pInPut = (unsigned char *)malloc(pow(2.0,18));
	if(length < pow(2.0,30))
	{
	...
	else if(length >= pow(2.0,30))
	{
		while(readlength <= length)
		{
			_fseeki64(pFile,readlength, SEEK_SET);
			unsigned char *pInPut = (unsigned char *)malloc(pow(2.0,30));
			unsigned int step = (pow(2.0,30)<(length - readlength))?pow(2.0,30):(length - readlength);
			//Ctime();
			//cout << "step = " << step << endl;
			fread(pInPut, 1, step, pFile);
			MD5Update(pInPut, step);
			readlength = readlength + pow(2.0,30);
	...
	strcpy(pMd5,  szmd5);
	//cout << pFileName << "的MD5为: " << szmd5 << endl;
	return true;
}
